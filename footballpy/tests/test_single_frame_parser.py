...
class TestProcessLine(unittest.TestCase):
    ...
    def test_frame_data(self):
        ...
        self.assertEqual(res['frame'], 10008)

    def test_ball_data(self):
        res = sp.process_line(self.line) 
        ball = res['ball']
        self.assertEqual(ball[0], 10.01)
        self.assertEqual(ball[1], 11.02) 

    def test_player_data(self):
        res = sp.process_line(self.line)
        players = {x: res[x] for x in res if x not in {'frame','ball'}}
        ...
        self.assertEqual(players['p1'], (-18.49, 5.60))
        self.assertEqual(players['p2'], (-6.46, 28.93))
        self.assertEqual(players['p3'], (-20.18, -1.90))
        self.assertEqual(players['p21'], (33.50, 4.45))
        self.assertEqual(players['p26'], (-21.25, 6.65))
